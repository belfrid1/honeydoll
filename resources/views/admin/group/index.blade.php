                    ...
                    <div class="col-lg-12 d-flex justify-content-center">
                        ...
                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                            <label class="btn btn-primary active mr-2">
                                ...
                                <input type="radio" value="female" name="gender" autocomplete="off" {{ $gender == 'female' ? 'checked' : '' }}> {{__('Female')}}
                            </label>
                            <label class="btn btn-primary">
                                <i class="fas fa-male"></i>
                                <input type="radio" value="male" name="gender" autocomplete="off" {{ $gender == 'male' ? 'checked' : '' }}> {{__('Male')}}
                            </label>
                        </div>
                        <!-------------------- Female /Male  tab end -------------------->
                    </div>
                    ...
                    <div class="container">
                        <form action="{{ route('admin.group.product.store') }}" method="POST"> @csrf
                            <div class="form-group row justify-content-center">
                                <div class="col-lg-5">
                                    <select class="form-control select2bs4" name="group" data-placeholder="Choose Group" required>
                            ...
                            <div class="content pb-2" style="overflow: auto; white-space: nowrap;">
                                <div class="container-fluid" style="display: inline-block;">
                                    ...
                                    <div class="col-3 btn btn-outline-secondary text-center">
                                        <a href="{{ route('admin.group.product.remove',['id'=>$product->did]) }}" class="float-right"><i class="fas fa-times-circle btn-outline-danger"></i></a>
                                    ...
                                    {{-- <div class="col-lg-12 btn btn-outline-secondary text-center mx-3">
                                        <a href="#" class="float-right"><i class="fas fa-times-circle btn-outline-danger"></i></a>
                                        <h6 class="text-left">{{__('No Product Selected') }}</h6>
                                        <h6 class="text-left">{{__('Commission in $') }}</h6>
                                        <img src="{{ asset('public/assets/product/default/default.png') }}" alt="{{__('Product Name') }}" title="{{__('Product Name') }}" height="75px" width="">
                                        <h6 class="text-center">{{__('Shop Name') }}</h6>
                                        <h6 class="text-center">{{__('Product Price') }}</h6>
                                        <h6 class="text-center"><a href="#" target="_blank">{{__('Product URL') }}</a></h6>
                                    </div> --}}
                                    @endforelse
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <a href="{{ route('admin.group.product.destroy') }}" class="btn btn-warning float-left" type="clear" onclick="return confirm('Are you sure you want to clear all this products ?')">{{__('All Clear') }}</a>
                                <button class="btn btn-success float-right px-5" type="submit" onclick="return confirm('Are you sure you want to store these products to this group ?')">{{__('Save') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
...
@section('inpagejs')
    <script>
        $('input[type=radio][name=gender]').change(function() {
            window.location.href = "admin-group-product?gender=" + this.value;
        });
